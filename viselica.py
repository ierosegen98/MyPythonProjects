...
def play(word):
    ...
    while tries > 0 and not guessed:
        action = input("Угадаешь букву (1) или слово целиком (2)? ").strip()

        if action == '1':
            letter = input("Введи одну букву: ").upper()
            if not letter.isalpha() or len(letter) != 1:
                print("Ошибка: введи ОДНУ букву!")
                continue
            ...
            word_guess = input("Введи слово целиком: ").upper()
            if not word_guess.isalpha() or len(word_guess) != len(word):
                print("Ошибка: слово должно состоять только из букв и быть той же длины.")
                continue
            if word_guess in guessed_words:
...
playing = input("Хочешь сыграть в Виселицу?")
while playing == 'да':
    word = get_word()
    guessed = play(word)
    if guessed == True:
    ...
    if guessed == False:
        print('Ты проиграл! Загаданным словом было:', word)
    playing = input("Хочешь сыграть в Виселицу?")
